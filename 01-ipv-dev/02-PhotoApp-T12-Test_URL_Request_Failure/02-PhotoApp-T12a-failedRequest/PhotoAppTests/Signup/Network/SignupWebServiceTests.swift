...
final class SignupWebServiceTests: XCTestCase {
    ...
    func test_SignupWebService_WhenURLRequestFails_ReturnsErrorMessageDescription() {
        // GIVEN
        let expectation = self.expectation(
            description: "A failed request expectation"
        )
        
        // WHEN
        sut.signup(withForm: signupFormRequestModel) { signupResponseModel, error in
            // THEN
            let errorDescription = "A localized description of and error"
            XCTAssertEqual(
                error,
    ...
    } // test_SignupWebService_WhenURLRequestFails_ReturnsErrorMessageDescription()
